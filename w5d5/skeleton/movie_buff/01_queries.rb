...
def harrison_ford
  ...
  # Movie.select(:id, :title).joins(:castings).where(actors: {name: 'Harrison Ford'}).where.not(castings: {ord: 1})
  Movie.select(:id, :title).joins(:castings).having("actors.name = ? AND castings.ord != 1", 'Harrison Ford')
